...
class WaypointGraphGPU:
    """
    将 waypoint_graph 预处理为 GPU 常驻的稀疏出边表，并提供固定长度的批量最短路径生成功能。
    """
    def __init__(
        self,
    ...
    def _get_offset_masks(self, L: int) -> torch.Tensor:
        ...
        缓存为单个按需增长的张量，避免为每个 L 永久保留一份显存。
        """
        device = self.device
        L = int(L)
        K = int(max(1, (L - 1).bit_length()))
        cached = self._offset_masks_cache
        if cached is not None and cached.shape[0] >= K and cached.shape[1] >= L:
            return cached[:K, :L]
